...
| **Приложение TGonPC на Android** (`android-app/`, APK) | **Не работает** — код в репозитории, рабочей сборки нет |
| **Kivy / buildozer** (`mobile_app/`) | **Не работает** — то же самое |

Нужен Telegram **на телефоне через это приложение** — пока не выйдет. Можно попробовать **официальный** Telegram на телефоне с SOCKS5 на ПК в одной Wi‑Fi (`--lan`) — без гарантий, см. ниже.
...
| Хотите APK на телефон | **Не готово.** См. `android/README.md` — исходники есть, можно доработать форком |
...
Каталоги `android-app/`, `mobile_app/`, `android/` — **не работают** в текущем виде.

Исходники оставил в репозитории: если захотите — **`android/README.md`**, сборка и правки на вашей стороне. Готового APK нет.
